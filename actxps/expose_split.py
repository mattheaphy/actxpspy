...
from actxps.tools import relocate
import pandas as pd
...
def expose_split(self):
    ...
    units passed to `expose_split()`. Policy exposures will always be expressed
    in years. Downstream functions like `exp_stats()` and `exp_shiny()` will 
    require clarification as to which exposure basis should be used to summarize
    results.    
    ...
    xp.ExposedDF.expose_cy(toy_census, "2022-12-31").expose_split()
    ```
    """

    if not self.cal_expo:
        raise AssertionError('`expose_split()` can only be used with an ' +
                             ...
                             'calling `expose_split()`.')

    if len(self.trx_types) > 0:
        warn('Transactions have already been attached to this `ExposedDF`. ' +
             'This will lead to duplication of transactions after exposures ' +
             'are split. The appropriate order of operations is to call ' +
             '`add_transactions()` after `expose_split()`.')

    target_status = self.target_status
    default_status = self.default_status
    date_cols = self.date_cols
    expo_length = self.expo_length
    ...
    post_anniv['exposure'] = np.where(post_anniv.split, 1 - post_anniv.h, 1)
    ...
    self.exposure_type = 'split_' + expo_length
    self.data = data
