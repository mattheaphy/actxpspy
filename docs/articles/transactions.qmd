...
The `add_transactions()` method attaches transactions to an `ExposedDF` object, which contains a data frame with exposure-level records. For our example, we first need to convert `census_dat` into exposure records using `ExposedDF()`.^[See [Exposures](exposures.qmd) for more information on creating `ExposedDF` objects.] This example will use policy year exposures.
...
The `plot()` and `table()` methods create visualizations and summary tables from `TrxStats` objects. See See [Visualizations](visualizations.qmd) for full details on these functions.
